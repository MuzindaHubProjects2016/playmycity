...
if(isset($_POST['artist_signup'])){ //CHECK if there are any missing fields
  if(isset($_POST['artist_city'])){
    // do nothing;
}else{
  echo "<br> Please select a city <br>";
}
foreach($_POST as $key => $value){
  if(!$value){   // !$value means value is empty
...
while ($row = mysqli_fetch_assoc($artists)){
  ...
  if(!$exists){
    if($flag){
      ...
      $artist_lastname = $_POST['artist_firstname'];
      ...
      $sql = "INSERT INTO playmycity_artists VALUES ('','$artist_stagename', '$artist_firstname', '$artist_lastname', '', '', '$main_genre', '$artist_city', '' )";
